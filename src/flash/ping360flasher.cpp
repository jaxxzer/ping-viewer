...
bool Ping360Flasher::bl_read_device_id(uint16_t *device_id) {
  Ping360BootloaderPacket::packet_cmd_read_dev_id_t readDevId = packet_cmd_read_dev_id_init;
  Ping360BootloaderPacket::packet_update_footer(readDevId.data);
  bl_write_packet(readDevId.data);
  Ping360BootloaderPacket::packet_t ret = bl_wait_packet(RSP_DEV_ID, BL_TIMEOUT_DEFAULT_US);
  if (ret) {
...
bool Ping360Flasher::bl_read_version(packet_rsp_version_t *version) {
  ...
  Ping360BootloaderPacket::packet_t ret = bl_wait_packet(RSP_VERSION, BL_TIMEOUT_DEFAULT_US);
  if (ret) {
...
bool Ping360Flasher::bl_read_program_memory(uint8_t **data, uint32_t address) {
  ...
  Ping360BootloaderPacket::packet_t ret = bl_wait_packet(RSP_PGM_MEM, BL_TIMEOUT_READ_US);
  if (ret) {
...
int Ping360Flasher::port_read(char *data, int nBytes) {
